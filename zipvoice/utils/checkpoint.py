...
def load_checkpoint_copy_proj_three_channel_alter(
    ...
    if model is not None:
        ...
        if "token_embed.weight" in dst_state_dict and hasattr(model, "token_embed"):
            ckpt_weight = dst_state_dict["token_embed.weight"]
            model_weight = model.token_embed.weight
            if ckpt_weight.shape != model_weight.shape:
                logging.warning(
                    f"token_embed shape mismatch: checkpoint {ckpt_weight.shape} vs model {model_weight.shape}. "
                    f"Adapting embedding weights for fine-tuning."
                )
                min_vocab = min(ckpt_weight.shape[0], model_weight.shape[0])
                new_weight = model_weight.data.clone()
                new_weight[:min_vocab] = ckpt_weight[:min_vocab]
                dst_state_dict["token_embed.weight"] = new_weight
